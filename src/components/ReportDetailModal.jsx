 ...
 * Displays a saved medical report with edit capability
 */
const ReportDetailModal = ({ isOpen, onClose, report, onUpdateSuccess }) => {
  ...
  useEffect(() => {
    if (isOpen && report) {
      setMetadata(report.extracted_data?.metadata || {});
      setResults(report.extracted_data?.results || []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      formData.append('patient_id', report.patient);
      formData.append('healthcare_worker_id', report.healthcare_worker);
      formData.append('report_type', reportType);
      formData.append('extracted_data', JSON.stringify(editedExtractedData));
      // Note: We're not sending the image again unless user uploads a new one

      // Call the API to update
      const result = await updateExtractedData(report.id, formData);
      ...
      setErrorMessage('Failed to update report. Please try again.');
    } finally {
      setIsSaving(false);
    }
  };
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 md:p-6 border-b border-gray-200 bg-gray-50">
          <div className="flex-1">
            ...
            {/* Report Type - Editable when in edit mode */}
            <div className="mt-2 flex items-center gap-2">
              ...
              {isEditing ? (
                ...
                <span className="inline-block px-3 py-1 text-xs md:text-sm font-semibold text-blue-600 bg-blue-50 border border-blue-200 rounded-md">
                  {reportType}
                </span>
              )}
            </div>
          ...
          <div className="w-full md:w-3/5 overflow-y-auto space-y-4 md:space-y-6">
            ...
            {results.map((result, resultIndex) => (
              ...
              >
                ...
                {/* Result Data - Editable or Table View */}
                {isEditing ? (
                  ...
                  result.result_data && result.result_data.length > 0 && (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {result.result_data.map((data, dataIndex) => (
                            <tr key={dataIndex} className="border-b hover:bg-gray-50">
                              <td className="px-3 py-2 text-gray-800">{data.test_name || 'N/A'}</td>
                              <td className="px-3 py-2 text-gray-800 font-medium">{data.value || 'N/A'}</td>
                              <td className="px-3 py-2 text-gray-600">{data.unit || '-'}</td>
                              <td className="px-3 py-2 text-gray-600">{data.reference_range || '-'}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div className="mt-4">
                    {isEditing ? (
                      ...
                      <div className="p-3 bg-gray-50 rounded-md">
                        ...
                        <p className="text-sm text-gray-800 whitespace-pre-wrap">{result.text_data}</p>
                      </div>
                    )}
                  </div>
                )}
            ...
            {/* Add Result Section Button - Only in Edit Mode */}
            {isEditing && (
              <button
            ...
            {/* Report Metadata - Only in View Mode */}
            {!isEditing && (
          ...
          {isEditing ? (
            ...
            <>
              <button
                ...
                className="px-5 py-2.5 md:px-6 md:py-3 text-sm md:text-base font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-lg shadow-sm transition-colors"
              >
                Edit Report
              </button>
              <button
                onClick={onClose}
                className="px-5 py-2.5 md:px-6 md:py-3 text-sm md:text-base font-semibold text-white bg-blue-500 hover:bg-blue-600 rounded-lg shadow-md transition-colors"
              >
                Close
              </button>
            </>
